...
m3 <- lm(number_of_complaints ~  East_JLM_lines  + intercity_sevice + innercity_sevice+ passengersnumber_thousands
...
#######################################################################################
#to excel
coefficients <- results$coefficients  # This typically contains the coefficients and statistics
...
print(results)
write_xlsx(results_df, "outputs//linear_results_res3.xlsx")
...
m2 <- lm(number_of_complaints ~  East_JLM_lines  + intercity_sevice + innercity_sevice+ passengersnumber_thousands
 + Directness_measurements + percent_problematic_trips + Settlements_lines + InJerusalem_lines
,  data = df2)
results <- summary(m2)
...
results_df <- results_df[, c("Coefficient", colnames(results_df)[-ncol(results_df)])]
print(results)
write_xlsx(results_df, "outputs//linear_results_res2.xlsx")
...
m1 <- lm(number_of_complaints ~  rush_morning+rush_afternoon+East_JLM_lines  + intercity_sevice 
...
results_df <- results_df[, c("Coefficient", colnames(results_df)[-ncol(results_df)])]
print(results)
write_xlsx(results_df, "outputs//linear_results_res1.xlsx")